...
public class SpriteStore {
    ...
    public SpriteStore() {
		...
		TemplateClass.instrum(28, "Assign", "SpriteStore.SpriteStore().spriteMap: ",spriteMap.toString());
    }
    ...
    public Sprite loadSprite(String resource) throws IOException {
		...
		if (result == null) {
            TemplateClass.instrum(44, "IF Statement", "SpriteStore.loadSprite().result: ",String.valueOf(result));
            result = loadSpriteFromResource(resource);
			TemplateClass.instrum(45, "Assign", "SpriteStore.loadSprite().result: ",result.toString(), "SpriteStore.loadSprite().resource: ",resource.toString());
            spriteMap.put(resource, result);
			TemplateClass.instrum(46, "Assign", "SpriteStore.loadSprite().spriteMap: ",spriteMap.toString(), "SpriteStore.loadSprite().resource: ",resource.toString(), "SpriteStore.loadSprite().result: ",result.toString());
        }
        return result;
    }
    ...
    private Sprite loadSpriteFromResource(String resource) throws IOException {
        try (InputStream input = SpriteStore.class.getResourceAsStream(resource)) {
            TemplateClass.instrum(62, "IF Statement", "SpriteStore.loadSpriteFromResource().input: ",input.toString());
			if (input == null) {
