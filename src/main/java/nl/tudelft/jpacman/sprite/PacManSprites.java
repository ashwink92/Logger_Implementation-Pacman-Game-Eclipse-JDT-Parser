...
public class PacManSprites extends SpriteStore {
    ...
    public AnimatedSprite getPacManDeathAnimation() {
		...
		TemplateClass.instrum(70, "Assign", "PacManSprites.getPacManDeathAnimation().animation: ",animation.toString());
    ...
    private Map<Direction, Sprite> directionSprite(String resource, int frames) {
        ...
        Sprite baseImage = loadSprite(resource);
        TemplateClass.instrum(88, "For Statement", "PacManSprites.directionSprite().DIRECTIONS: ",DIRECTIONS.toString(), "PacManSprites.directionSprite().length: ", String.valueOf(DIRECTIONS.length));
		for (int i = 0; i < DIRECTIONS.length; i++) {
			...
			TemplateClass.instrum(93, "Assign", "PacManSprites.directionSprite().animation: ",animation.toString());
            sprite.put(DIRECTIONS[i], animation);
			TemplateClass.instrum(94, "Assign", "PacManSprites.directionSprite().sprite: ",sprite.toString(), "PacManSprites.directionSprite().DIRECTIONS: ",DIRECTIONS.toString(), "PacManSprites.directionSprite().i: ",String.valueOf(i), "PacManSprites.directionSprite().animation: ",animation.toString());
        }
